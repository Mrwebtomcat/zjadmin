<template>
	<section>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm1" label-width="150px" :rules="editFormRules" ref="editForm">
	...
	export default {
		data() {
			return {
				...
				editFormRules: {
					n_time: [
						{ required: true, message: '请设置会员月份', trigger: 'blur change' }
					],
					vc_daymoney: [
						{ required: true, message: '请设置会员日均消费金额', trigger: 'blur change' }
					],
					n_money: [
						{ required: true, message: '请设置会员充值月份对应金额', trigger: 'blur change' }
					]
		...
		methods: {
			...
			getVipList() {
				//NProgress.start();
				connetAction.ajaxPost(httpUrl['nvIndex'],{type:1})
				.then((res)=>{
					console.log(res)
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							connetAction.ajaxPost(httpUrl['addSetvip'],para)
							.then((res)=>{
								this.$message({
									message: res.message,
									type: 'success'
